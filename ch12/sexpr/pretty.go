...
// 这个文件实现了 Derek C. Oppen's 中描述的算法
// 1979 Stanford technical report, "Pretty Printing".
// 1979 年斯坦福技术报告，“Pretty Printing”。
...
type token struct {
	kind rune //  one of "s ()" (string, blank, start, end)	// "s ()" 之一（字符串、空白、开始、结束）
	str  string
	size int
}

type printer struct {
	tokens []*token // FIFO buffer // 先进先出缓冲器
	stack  []*token // stack of open '	' and '(' tokens // 堆栈打开的“”和“（”标记
	rtotal int      // total number of spaces needed to print stream	// 打印流所需的空间总数

	bytes.Buffer
	indents []int
	width   int // remaining space	// 剩余空间
}
...
func pretty(p *printer, v reflect.Value) error {
	...
	default: // float, complex, bool, chan, func, interface
		return fmt.Errorf("unsupported type: %s", v.Type()) // 不支持的类型
	}
	return nil
}
